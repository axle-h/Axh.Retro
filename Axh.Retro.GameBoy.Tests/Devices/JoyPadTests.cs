namespace Axh.Retro.GameBoy.Tests.Devices
{
    ...
    public class JoyPadTests
    {
        ...
        private void AssertRegisterMatrix(Expression<Func<IJoyPad, bool>> buttonExpression, byte regOut, byte expectedMask)
        {
            ...
            var setLambda = Expression.Lambda<Action<IJoyPad>>(Expression.Assign(property, Expression.Constant(true)), joyPadExpression).Compile();
            var resetLambda = Expression.Lambda<Action<IJoyPad>>(Expression.Assign(property, Expression.Constant(false)), joyPadExpression).Compile();

            // Assert button pressed
            setLambda(this.joyPad);
            this.joyPad.Register = regOut;
            // Button is pressed then flag is reset
            Assert.IsTrue((this.joyPad.Register & expectedMask) == 0);

            // Assert button released
            resetLambda(this.joyPad);
            this.joyPad.Register = regOut;
